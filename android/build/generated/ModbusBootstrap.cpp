...
#define TAG "de.appwerft.modbus"

using namespace v8;

static Persistent<Object> bindingCache;

static void Modbus_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Modbus.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::ModbusBindings::lookupGeneratedInit(
...
static void Modbus_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Modbus_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Modbus_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::ModbusBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ModbusBinding = {
	"de.appwerft.modbus",
	Modbus_init,
	Modbus_dispose
};
...
Java_de_appwerft_modbus_ModbusBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.modbus", &ModbusBinding);
	titanium::KrollBindings::addExternalLookup(&(::ModbusBindings::lookupGeneratedInit));
}
